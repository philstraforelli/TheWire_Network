...
characters <- read_csv("characters_data.csv") |> 
  mutate(category = case_when(
    category %in% c("Barksdale Crew", "Marlo Crew", "Other Gang") ~ "Street",
    name == "Roland Pryzbylewski" ~ "School",
    TRUE ~ category),
    category = fct_relevel(category, "Law", "Police", "Politician", "Street", "Stickup", "Addict", "Civilian"))
...
nodes_colours <- brewer.pal(8, "Set1")
nodes_colours[8] <- "#808080"

names(nodes_colours) <- c("Law", "Police", "Politician", "Street", "School", "Stickup", "Addict", "Civilian")
text_colours <- darken(nodes_colours, 0.5)

img <- image_read("www/TheWire-Logo_CR.png")

graph_s4 <- ggraph(net_tidy) + 
  ...
  labs(fill = "", colour = "", subtitle = "Season Four", caption = "/u/AllezCannes") +
...
ggsave("season4.png", graph_s4, device = "png", dpi = 450, width = 13, height = 10, units = "in", scale = 1.3)
